...
# Plan — `device_manager` trae su vista (`ui/`), sus datos de demo (`seed/`) y su demo (`web/`)

## 1. Por qué

La vista de este módulo vivía en `veltylabs/mjosefa-cms/modules/device_manager` (la app de
...
5. **Qué borra.** `mjosefa-cms/modules/device_manager/` completo (lo borra el plan de
...
Archivos a copiar desde `_temp/mjosefa-cms/modules/device_manager/`:

| Origen | Destino | Cambios |
|---|---|---|
| `module.go` | `ui/module.go` | `package device_manager` → `package ui`. `ID = "device_manager"`, `Label = "Equipos"`. |
| `browser.go` | `ui/browser.go` | `package ui`. Comentarios que nombran `modules/browser.go`/`server.go` → "the app's registry". |
| `svg.go` | `ui/svg.go` | `package ui`, conserva `//go:build !wasm`. El literal `svg.Icon("device_manager")` → `svg.Icon(ID)`. |

`server.go` **no** se copia: construir el backend es trabajo del composition root de
cada app (y de `web/client.go` aquí). Su adaptador `deviceLocator` (que vive hoy en
`_temp/mjosefa-cms/modules/staff_manager/server.go`) se reemplaza por `IPLocator` (etapa 3).
...
Copiar los tres archivos de §3 con sus cambios. `Browser` conserva su firma:
`func Browser(caller router.Caller, ids model.IDGenerator, tenantID string) (platformd.UIModule, error)`.

### Etapa 3 — `IPLocator` en el dominio (`locator.go`, nuevo, paquete raíz)
...
Test `tests/locator_test.go`: con `orm.New(mem.New())`, crear un dispositivo con IP
`192.168.1.10` → `FindByIP("192.168.1.10")` devuelve su id y `true`; una IP
desconocida → `""`, `false`; la misma IP en **otro** tenant → `""`, `false`.

### Etapa 4 — `seed/seed.go`
...
Filas (`TenantId: tenantID`, `IsActive: true` en todas; `Type` siempre por constante):

| Name | Ip | Type | Location |
|---|---|---|---|
| `Recepción` | `192.168.1.10` | `DeviceTypeComputer` | `Recepción` |
| `Box 1` | `192.168.1.11` | `DeviceTypeComputer` | `Box 1` |
| `Impresora` | `192.168.1.20` | `DeviceTypePrinter` | `Recepción` |

Exportar las IPs como constantes (`ReceptionIP = "192.168.1.10"`, `Box1IP`,
`PrinterIP`): la semilla de `staff_manager` las usa.

### Etapa 5 — `web/client.go`

Esqueleto de §2.4 con:

```go
dm, err := devicemanager.New(db, devicemanager.Deps{IDs: ids, Publisher: broker, TenantID: demoTenantID})
// seed.Load(dm, demoTenantID)
caller := loopback.WithTenant(demoTenantID, dm)
v, err := ui.Browser(caller, ids, demoTenantID)
```

### Etapa 6 — tests de la vista (`tests/ui_view_test.go`, nuevo, sin tag)

mjosefa-cms no tiene tests de vista para este módulo. Crear uno: con un `router.Caller`
falso (reutilizar el fake de `tests/setup_test.go` si ya implementa
`Call(op string, args model.Encodable, out model.Decodable, done func(error))`; si no,
crear `tests/ui_fakes_test.go` con uno que registre las ops llamadas en un slice),
`ui.Browser(fake, ids, "t")` → sin error, `ModelName() == ui.ID`, `Label() == ui.Label`,
y `Init(nil)` sobre su `View()` provoca una llamada cuyo nombre termina en `list_devices`
(las ops van calificadas por módulo: `device_manager.list_devices`).

Además, `tests/seed_test.go`: `seed.Load` sobre `orm.New(mem.New())` devuelve 3
dispositivos sin error (fija que la semilla pasa la validación del modelo).

### Etapa 7 — `go.mod` y README
...
1. `go get webtyp.com/layout@latest webtyp.com/components@latest webtyp.com/dom@latest
   ...
   webtyp.com/unixid@latest` y `go mod tidy`. Sin `replace` (ver §2.2).
...
| 3 | `IPLocator` | `locator.go`, `tests/locator_test.go` |
| 4 | semilla | `seed/seed.go` |
| 5 | demo | `web/client.go` |
| 6 | tests | `tests/ui_view_test.go`, `tests/seed_test.go` |
| 7 | deps + README | `go.mod`, `go.sum`, `README.md` |
| 8 | borrar `_temp/` | `_temp/` |
